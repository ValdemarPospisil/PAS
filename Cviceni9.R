...
syst1 <- Stulong$syst1
hist(syst1,col="azure", border="darkblue", main="Histogram pro hladinu tlaku", xlab="Tlak")
PlotQQ(syst1, pch=19)
Skew(syst1)
Kurt(syst1)

descdist(syst1 ,discrete=FALSE, boot=1000)

(fit1 <- fitdist(syst1,"weibull"))
(fit2 <- fitdist(syst1,"gamma"))
(fit3 <- fitdist(syst1,"lnorm"))

data.frame(distr=c("Weibull","Gamma","Lognorm"),
           AIC=c(fit1$aic,fit2$aic,fit3$aic),
           BIC=c(fit1$bic,fit2$bic,fit3$bic)
)
...
(fit2 <- fitdist(cukr,"exp"))
(fit3 <- fitdist(cukr,"logis"))

data.frame(distr=c("Norm","Exp","Lognorm"),
           AIC=c(fit1$aic,fit2$aic,fit3$aic),
           BIC=c(fit1$bic,fit2$bic,fit3$bic)
)
...
PlotQQ(cukr)
PlotQQ(cukr, qdist=function(p) qexp(p,rate=coef(fit3)[1]))
