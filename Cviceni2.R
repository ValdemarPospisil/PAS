...
ggplot(Cars93, aes(x = as.numeric(Cylinders))) +
  geom_freqpoly(binwidth = 1, color = "red", size = 1) +  # Create the frequency polygon
  geom_point(stat = "bin", binwidth = 1, color = "darkgreen", size = 3) +  # Add points at each frequency
  theme_minimal() +
  labs(title = "Frekvenční polygon počtu válců",
       x = "Počet válců",
       y = "Absolutní četnosti")
...
barplot(ac,col=2:4,main="Počet cestujících v prodavaných vozech",ylab="Absolutni cetnosti",ylim=c(0,max(ac)+3))
x <- barplot(ac,plot=F)[,1]
text(x,ac,labels=ac,pos=3)
...
# zmena typu promenne kvuli popiskum na y-ove ose v grafu
plot(x.val,ac.n,type="h",lwd=3,col="darkgreen",main="Frekvencni polygon",
     xlab="Počet cestujících",ylab="Absolutni cetnosti")
x.val2<-min(x.val):max(x.val)
ac2<-rep(0,length(x.val2))
for(i in 1:length(x.val2)){
  for(j in 1:length(x.val)){if(x.val2[i]==x.val[j]){ac2[i]<-ac[j]}}
