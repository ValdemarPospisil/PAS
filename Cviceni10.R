...
tapply(hmotnost, kojeno, MeanCI)

MeanDiffCI(hmotnost ~ kojeno)
...
BinomCI(table(kojeno)[1], sum(table(kojeno)), method = "wald", conf.level = 0.90)
...
kojeno <- Kojeni$Koj24
(tab <- table(kojeno, mesto))
x1 <- tab[1,1]
x2 <- tab[1,2]
n1 <- tab[1,1] + tab[2,1]
n2 <- tab[1,2] + tab[2,2]
BinomDiffCI(x1,n1,x2,n2, method="wald", conf.level = 0.99)
